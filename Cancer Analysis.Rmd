...
dt$menopaus<-as.factor(dt$menopaus)
dt$density<-as.factor(dt$density)
dt$nrelbc<-as.factor(dt$nrelbc)
dt$brstproc<-as.factor(dt$brstproc)
dt$lastmamm<-as.factor(dt$lastmamm)
dt$surgmeno<-as.factor(dt$surgmeno)
dt$hrt<-as.factor(dt$hrt)
dt$invasive<-as.factor(dt$invasive)
...
```{r}
men<-c('No','Yes','Unk')
p5 <- ggplot(dt, aes(x=menopaus)) +
    geom_bar(fill="powderblue") +
    ggtitle("Menopaus") + theme(axis.text.x = element_text(angle = 65, hjust = 1))+ scale_x_discrete(labels=c(men))


p6 <- ggplot(dt, aes(x=density)) +
    geom_bar(fill="powderblue") +
    ggtitle("Density")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))

p7 <- ggplot(dt, aes(x=nrelbc)) +
    geom_bar(fill="powderblue") +
    ggtitle("nrelbc")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))

p8 <- ggplot(dt, aes(x=brstproc)) +
    geom_bar(fill="powderblue") +
    ggtitle("Previous Breast Proc")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))+scale_x_discrete(labels=c(men))      
lm<-c('neg','falpos','unk')
p9 <- ggplot(dt, aes(x=lastmamm)) +
    geom_bar(fill="powderblue") +
    ggtitle("Last Mamm")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))+scale_x_discrete(labels=c(lm)) 
su<-c('nat','sur','unk')
p10 <- ggplot(dt, aes(x=surgmeno)) +
    geom_bar(fill="powderblue") +
    ggtitle("BMI")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))+scale_x_discrete(labels=c(su)) 


p11 <- ggplot(dt, aes(x=hrt)) +
    geom_bar(fill="powderblue") +
    ggtitle("hrt")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))+scale_x_discrete(labels=c(men)) 

p12 <- ggplot(dt, aes(x=invasive)) +
    geom_bar(fill="powderblue") +
    ggtitle("Invasive")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))+scale_x_discrete(labels=c(men))   


p13 <- ggplot(dt, aes(x=cancer)) +
    geom_bar(fill="powderblue") +
    ggtitle("Cancer")+ theme(axis.text.x = element_text(angle = 65, hjust = 1))+scale_x_discrete(labels=c(men))  
...
dt$menopaus<-as.integer(dt$menopaus)
dt$density<-as.integer(dt$density)
dt$nrelbc<-as.integer(dt$nrelbc)
dt$brstproc<-as.integer(dt$brstproc)
dt$lastmamm<-as.integer(dt$lastmamm)
dt$surgmeno<-as.integer(dt$surgmeno)
dt$hrt<-as.integer(dt$hrt)
dt$invasive<-as.integer(dt$invasive)
dt$cancer<-as.integer(dt$cancer)
